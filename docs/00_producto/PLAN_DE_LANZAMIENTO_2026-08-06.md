...
| **L** | **El proyecto de Supabase que hoy es producción se llamó `beautyos-dev`.** `eogppgbdnwxdtcbctaol` fue elegido en **D-027 el 20-jul como el entorno NO productivo** para ensayar los tramos D3.2 y D3.4 — y hoy es el que la aplicación tiene escrito en `lib/main.dart:48`, el que respalda `scripts/respaldo_supabase.ps1` y el único que existe. Verificado el 09-ago | 🔴 **Es una trampa esperando.** Alguien que lea D-027 a D-033 y ejecute una prueba destructiva "contra dev" la ejecutaría **contra producción**. No hay nada que arreglar en el código: lo que hay que arreglar es la creencia. Cuando se cree el segundo proyecto para la restauración de ensayo (hallazgo G), **ese** será el no productivo, y hay que nombrarlo de forma que no se confunda con este |
