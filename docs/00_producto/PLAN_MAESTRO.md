...
| 8.4 | **Blindar Edge Functions y sincronizar configuración (Hallazgo U):** `verify_jwt = true` para funciones con sesión de usuario y declarar funciones de Smart Checkout en `config.toml` | 🤖 | ⬜ |
| 8.5 | **Clarificar pantalla de acceso para colaboradores invitados (Hallazgo S):** separar camino de dueño ("Registra tu salón") frente a empleado invitado ("Inicia sesión con tu correo") | 🤖 | ⬜ |
| 8.6 | **Gestión de sedes para usuarios de equipo multi-sede (Hallazgo V):** interfaz en Usuarios y RPC para asignar/revocar acceso a sedes (`branch_memberships`) a un colaborador existente | 🤖 | ⬜ |
| 8.7 | **Reenvío de correos de soporte `hola@salonymas.com` (Idea I-12):** activación en Cloudflare Email Routing para recibir respuestas de salones | 👥 | ⬜ |
