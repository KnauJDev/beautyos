...
| 9.29 | 🔴 **Nadie puede marcar un negocio como de prueba.** D-120 creó `is_demo` con un `update` a mano dentro de una migración y **no dejó ni RPC ni botón**. Consulta del 07-sep: **los tres negocios en `is_demo = false`** — Naguara (que el script de ePayco revirtió), Prueba Barbería Elite y Exportadora, que nacieron con el `default false`. Las métricas del SaaS llevan meses contando ensayos (D-220) | 🤖 + 👤 | 🔄 **Backend cerrado 07-sep (D-225).** Nace `platform_set_tenant_demo` (solo dueño de plataforma, reversible, con rastro) y la migración `20260907140000` marca los tres. **Falta el botón en el Panel de Plataforma.** ✅ **APLICADA Y VERIFICADA el 07-sep:** los tres negocios en `is_demo = true`, y `anon` no alcanza la función. ✅ **CERRADO ENTERO 07-sep.** El Panel tiene su fila *"Negocio de ensayo"* con botón para marcar y desmarcar, solo visible para el dueño de plataforma y con aviso de las consecuencias antes de aplicar. `flutter analyze` 0/0 y **388 pruebas en verde** |
...
| 9.35 | **El Panel no deja ver el estado de cada sede.** Lista negocios, y las sedes salen solo como número. Tras un pago de sede, el panel se ve igual antes y después | 🤖 | ⬜ Una sección de sedes dentro de la ficha del negocio, con estado, precio y vencimiento de cada una. **Después del 9.34:** una cifra falsa es peor que una pantalla que falta |
