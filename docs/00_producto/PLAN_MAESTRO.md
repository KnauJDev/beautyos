...
| 8.18 | 🔴 **Bloque 2 "Pulido Multi-Sede y Alertas de Suscripción": tope de equipo por sede y alertas de vencimiento por sede.** Los dos pendientes que D-189 (apartado del tope) y el HANDOFF de D-195 (apartado 3.1, alertas) dejaron escritos, ahora que la Etapa 2/3 de D-188 ya existe | 🤖 | 🔄 **Escrito 02-sep (D-196), pendiente de aplicar.** `private.beautyos_require_team_limit` multiplica el tope (9) por las sedes activas: la principal cuenta si el negocio está `entitled` (cero regresión), las secundarias por su propio `branch_subscriptions.status`. `send-subscription-expiry-alerts` agrupa alertas de negocio y de sedes secundarias en un solo correo por dueño. Migración `20260902200000` y Control `206` (10 casos) escritos; **faltan aplicar la migración con `aplicar_sql.ps1` y desplegar la Edge Function** — ambos piden credenciales interactivas que esta sesión no tiene cómo suministrar. `flutter analyze` 0/0 y 297/297 pruebas en verde (sin cambios en Flutter: el bloque es de base de datos y Edge Function). No se construyó suspensión automática por sede: el encargo pedía alertas, no suspensión |
