...
| 8.27 | **Recuperar lo que el respaldo escondía 2,5 semanas: chip del número de venta, botón de WhatsApp y búsqueda por teléfono en la lista de Tickets.** Sale de D-203 | 🤖 | ⏳ **Pendiente, y necesita un dato de la base primero.** Pasar a `get_ticket_board_list_v2` con un rango de fechas los recupera los tres, pero esa RPC filtra `scheduled_at is not null` y el respaldo no: **hay que contar cuántos tickets tienen `scheduled_at` nulo antes de tocarlo**, porque un ticket que no se ve es un ticket que no se cobra. La alternativa limpia es una migración que haga a la RPC aceptar nulos y no excluirlos. Cierra además el tercio pendiente de **TL-09** (la consulta trae el historial completo) |
