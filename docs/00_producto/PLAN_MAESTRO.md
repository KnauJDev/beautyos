...
| 9.29 | 🔴 **Nadie puede marcar un negocio como de prueba.** D-120 creó `is_demo` con un `update` a mano dentro de una migración y **no dejó ni RPC ni botón**. Consulta del 07-sep: **los tres negocios en `is_demo = false`** — Naguara (que el script de ePayco revirtió), Prueba Barbería Elite y Exportadora, que nacieron con el `default false`. Las métricas del SaaS llevan meses contando ensayos (D-220) | 🤖 + 👤 | 🔄 **Backend cerrado 07-sep (D-225).** Nace `platform_set_tenant_demo` (solo dueño de plataforma, reversible, con rastro) y la migración `20260907140000` marca los tres. **Falta el botón en el Panel de Plataforma.** ✅ **APLICADA Y VERIFICADA el 07-sep:** los tres negocios en `is_demo = true`, y `anon` no alcanza la función. **Falta el botón en el Panel de Plataforma** |
