...
| **AA** | **`send-invitation-email` y `send-low-stock-alert` quedaron fuera de la unificación de claves.** Siguen leyendo `SUPABASE_ANON_KEY` a mano mientras las cuatro funciones del cobro pasaron a `_shared/supabase_keys.ts` (D-214). **Si el proyecto desactiva las claves heredadas, fallan igual que fallaron las del cobro.** Es el mismo patrón del hallazgo T: se arregla una función y su gemela se queda atrás. Encontrado el 06-sep | Fase 8. Mismo arreglo: importar el módulo compartido |
