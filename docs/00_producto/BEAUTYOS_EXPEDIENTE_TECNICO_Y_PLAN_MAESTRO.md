...
**Versión:** 1.5
**Estado:** Rector aprobado — Fase 1 en implementación; Tramos A, B y C aprobados en producción; D1–D2 y D3.2–D4.1 verificados localmente, sin modificar producción
...
**Continuar con D4.2 — seleccionar un entorno Supabase no productivo conectable:** D4.1 confirmó que el repositorio no tiene configuración ni credenciales de conexión y que la pila CLI local no está disponible. La matriz de cliente heredado, privilegios y aislamiento está definida, pero no debe ejecutarse remotamente hasta identificar inequívocamente un destino no productivo y una conexión segura de solo lectura. Las alertas operativas continúan pausadas.
