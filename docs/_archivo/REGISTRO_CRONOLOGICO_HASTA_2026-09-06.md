# Registro cronológico de documentos y migraciones (hasta el 06-sep-2026)

> **Archivado el 06-sep-2026 (D-216, paso 9.1).** Esto vivió hasta hoy como el §6 de
> `docs/README.md`: 248 entradas mantenidas **a mano**, que había que actualizar en cada
> cambio. Se archiva por la misma razón por la que se fundieron los siete planes (D-126)
> y por la que las reglas dejaron de copiarse en seis sitios (D-131): **un registro
> duplicado no se refuerza, diverge.**
>
> Y había divergido por tres lados a la vez: marcaba **dos** handoffs distintos como
> "vigente" y ninguno lo era (se quedó en D-211, cuando ya iba por D-215), decía 211
> decisiones cuando había 215, y 75 documentos archivados cuando había 124.
>
> **Lo reemplaza `git log`**, que lleva ese mismo historial solo, no se desincroniza nunca
> y sabe además qué cambió exactamente en cada archivo. El §6 del README explica cómo leerlo.
>
> **No manda sobre nada.** Se conserva porque las anotaciones entre paréntesis explican el
> porqué de algunos archivos con palabras que un mensaje de commit no trae. Cada entrada
> cita su `D-XXX`: el porqué completo está en `REGISTRO_DE_DECISIONES.md`, que sí manda.
...
246. `_archivo/handoffs/HANDOFF_SalonyMas_2026-09-04_D210.md` (handoff de D-210, reemplazado)
247. `../lib/services/monitoreo_service.dart`, `../lib/pages/agenda_page.dart`, `../lib/pages/platform_panel_page.dart` y `../test/monitoreo_service_test.dart` (**paso 8.34**: guardián de excepciones muertas de la lista blanca, sanitización de fijos (60x) y errores Postgres, y prevención de re-disparos en FutureBuilder — D-211)
248. `HANDOFF/HANDOFF_SalonyMas_2026-09-04_D211.md` (**handoff vigente**, con el prompt exacto para retomar)
