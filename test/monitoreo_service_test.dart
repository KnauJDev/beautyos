...
void main() {
  ...
  group('Sanitización de PII (Ley 1581) en MonitoreoService', () {
    ...
    test('oculta números de teléfono de 7 a 15 dígitos y con espacios (celulares y fijos)', () {
      final texto =
          'WhatsApp al 300 123 4567 o al fijo 601 765 4321 o sin espacios 6017654321 internacional +57 310 987 6543';
      final sanitizado = MonitoreoService.taparDatosSensibles(texto);

      expect(sanitizado, isNot(contains('300 123 4567')));
      expect(sanitizado, isNot(contains('601 765 4321')));
    ...
    test('oculta detalles de nombres y claves en errores de base de datos Postgres (Key constraint)', () {
      final texto =
          'PostgrestException(message: duplicate key value violates unique constraint "clients_name_key", details: Key (nombre)=(María Restrepo) already exists.)';
      final sanitizado = MonitoreoService.taparDatosSensibles(texto);

      expect(sanitizado, isNot(contains('María Restrepo')));
      expect(sanitizado, contains('Key ([campo])=([valor oculto])'));
    });
    ...
    test('_limpiar sanea message, exceptions y breadcrumbs en un SentryEvent completo', () {
      final eventoOriginal = SentryEvent(
        ...
        exceptions: [
          SentryException(
            ...
            value: 'Error en base con cliente maria@beauty.com o tel 3151234567 y Key (nombre)=(Carlos Perez)',
          ),
        ],
        breadcrumbs: [
          Breadcrumb(
            message: 'debugPrint: PostgrestException con titular carlos@empresa.co fijo 601 888 9999',
          ),
        ],
      );
      ...
      expect(exLimpia?.value, isNot(contains('Carlos Perez')));
      expect(exLimpia?.value, contains('[correo oculto]'));
      expect(exLimpia?.value, contains('[número oculto]'));
      expect(exLimpia?.value, contains('Key ([campo])=([valor oculto])'));
      ...
      expect(bLimpio?.message, isNot(contains('601 888 9999')));
      expect(bLimpio?.message, contains('[correo oculto]'));
      expect(bLimpio?.message, contains('[número oculto]'));
    });
  });

  group('Guardián estricto de excepciones no expuestas en UI (D-209, D-210, D-211)', () {
    ...
    test('ninguna pantalla de cara al salón le enseña snapshot.error en crudo al usuario', () {
      ...
      final archivos = <File>[
        ...Directory('lib/pages').listSync(recursive: true).whereType<File>(),
    ...
    test('las excepciones de la lista blanca siguen haciendo falta', () {
      for (final entry in permitidos.entries) {
        final archivo = File(entry.key);
        expect(
          archivo.existsSync(),
          isTrue,
          reason: 'El archivo en lista blanca ${entry.key} no existe.',
        );
        final contenido = archivo.readAsStringSync();
        expect(
          regex.hasMatch(contenido),
          isTrue,
          reason:
              '${entry.key} ya no contiene ninguna referencia a snapshot.error. '
              'La excepción en la lista blanca ha quedado muerta y debe retirarse. '
              'Motivo que tenía: ${entry.value}',
        );
      }
    });
  });
