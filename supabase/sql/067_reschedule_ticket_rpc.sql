-- Paso 1027: reprogramar sin cancelar ni crear un ticket nuevo.
-- Conserva estado, servicios y estilistas; deja evidencia en ticket_history.
...
begin
  ...
  if v_ticket.status not in ('apartado', 'confirmado', 'en_espera') then
    raise exception 'Solo se puede reprogramar un ticket apartado, confirmado o en espera.';
  end if;
  ...
  conflicts as (
    ...
    join occupied_assignments oa
     ...
     and p_new_scheduled_at < oa.scheduled_at + (oa.duration_minutes * interval '1 minute')
     and p_new_scheduled_at + (ta.duration_minutes * interval '1 minute') > oa.scheduled_at
  )
  ...
  if v_conflicting_stylist_names is not null then
    raise exception 'La nueva hora presenta un choque de agenda para: %.', v_conflicting_stylist_names;
  end if;
